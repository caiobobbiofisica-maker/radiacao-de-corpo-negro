...
const getBlackBodyColor = (temp) => {
  if (temp < 800) return '#000000'; if (temp < 1200) return '#ff4500'; if (temp < 1800) return '#ff6600';
  if (temp < 2500) return '#ff8800'; if (temp < 3500) return '#ffaa00'; if (temp < 5000) return '#ffdd00';
  return '#ffffff';
};
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        <div className="bg-black/40 border border-white/10 rounded-lg p-6 space-y-4">
          ...
          <div>
            ...
            <input id="temperature-slider" type="range" min="300" max="7000" value={temperature} onChange={(e) => setTemperature(parseInt(e.target.value))} className="w-full h-2 bg-gray-700 rounded-lg appearance-none cursor-pointer mt-2"/>
          </div>
          <div className="flex flex-col items-center justify-center pt-4">
            <div className="w-24 h-24 rounded-full border-4 border-white/10 transition-all duration-300" style={{ backgroundColor: getBlackBodyColor(temperature), boxShadow: `0 0 35px ${getBlackBodyColor(temperature)}` }}></div>
      ...
      <div className="bg-black/40 border border-white/10 rounded-lg p-8">
        ...
        <p className="text-gray-300 text-lg leading-relaxed mb-4">Quando aquecemos qualquer objeto, ele emite luz - isso é chamado de <strong className="text-orange-400">radiação térmica</strong>. A cor e a intensidade dessa luz dependem diretamente da temperatura do objeto. Este fenômeno está presente em toda parte: desde a brasa do carvão até as estrelas distantes no universo.</p>
        <p className="text-gray-300 text-lg leading-relaxed">Mas existe um tipo especial de objeto que emite radiação de forma perfeita: o <strong className="text-yellow-400">corpo negro</strong>. Apesar do nome, quando aquecido, ele brilha intensamente e de forma previsível.</p>
      </div>
      <div className="text-center">
        <button onClick={onNavigate} className="inline-flex items-center justify-center gap-2 bg-gradient-to-r from-orange-500 to-amber-500 hover:scale-105 transition-transform text-white px-8 py-3 text-lg font-semibold rounded-lg shadow-lg shadow-orange-500/30">
